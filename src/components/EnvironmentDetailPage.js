...
const EnvironmentDetailPage = ({
  ...
  const getResourceStats = () => {
    ...
    const helmCount = Object.values(services.eks?.helmCharts || {}).filter(c => c?.enabled).length;
  ...
  const MetricCard = ({ icon: Icon, title, value, subtitle, trend, color = 'teal' }) => (
    <div className={`rounded-xl border backdrop-blur-sm transition-all duration-200 hover:scale-[1.02] ${
      isDark
        ...
        <div className={`rounded-xl border backdrop-blur-sm ${
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              ...
              <button className={`w-full p-3 rounded-lg flex items-center space-x-3 transition-colors ${
                isDark
