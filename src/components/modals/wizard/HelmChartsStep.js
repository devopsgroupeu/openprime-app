...
const HelmChartsStep = ({ newEnv, setNewEnv, onEditHelmValues }) => {
  ...
  const getEnabledK8sService = () => {
    if (newEnv.services?.eks?.enabled) return { name: 'eks', displayName: 'Amazon EKS', provider: 'AWS' };
    if (newEnv.services?.aks?.enabled) return { name: 'aks', displayName: 'Azure AKS', provider: 'Azure' };
    if (newEnv.services?.gke?.enabled) return { name: 'gke', displayName: 'Google GKE', provider: 'Google Cloud' };
    if (newEnv.services?.kubernetes?.enabled) return { name: 'kubernetes', displayName: 'Kubernetes', provider: 'On-Premise' };
    return null;
  };
  ...
  const getEnabledChartsCount = () => {
    ...
    return Object.values(newEnv.services[k8sService.name].helmCharts).filter(chart => chart?.enabled).length;
  };
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className={`text-sm ${
          ...
          This step is only available when a Kubernetes service (EKS, AKS, GKE, or Kubernetes) is enabled.
        </p>
      </div>
    );
  }
      ...
      {getEnabledChartsCount() > 0 && (
        <div className={`p-4 rounded-lg border ${
          ...
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium">
              {getEnabledChartsCount()} Helm chart{getEnabledChartsCount() !== 1 ? 's' : ''} will be deployed to your {k8sService.displayName} cluster
            </span>
          </div>
        </div>
      )}
